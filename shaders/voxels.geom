...
void main() {
    ...
    EmitQuad(center + dx, dy, dz, max(dot(-light, vec3(1., 0., 0.)), 0.1));

    if ((gFaces[0] & 0x02) == 0 && !Culled(-dx))
    EmitQuad(center - dx, dz, dy, max(dot(-light, vec3(-1., 0., 0.)), 0.1));

    if ((gFaces[0] & 0x04) == 0 && !Culled(dy))
    EmitQuad(center + dy, dz, dx, max(dot(-light, vec3(0., 1., 0.)), 0.1));

    if ((gFaces[0] & 0x08) == 0 && !Culled(-dy))
    EmitQuad(center - dy, dx, dz, max(dot(-light, vec3(0., -1., 0.)), 0.1));

    if ((gFaces[0] & 0x10) == 0 && !Culled(dz))
    EmitQuad(center + dz, dx, dy, max(dot(-light, vec3(0., 0., 1.)), 0.1));

    if ((gFaces[0] & 0x20) == 0 && !Culled(-dz))
    EmitQuad(center - dz, dy, dx, max(dot(-light, vec3(0., 0., -1.)), 0.1));
}